...
OCR_SPACE_API_KEY = 'K82385935688957'  # Replace with your OCR.Space API key
...
def ocr_space_extract(image_path, api_key=OCR_SPACE_API_KEY, language='eng'):
    """
    Extracts raw text from an image using OCR.Space Free API.
    
    Args:
        image_path (str): Path to the image file (JPG/PNG, <1MB).
        api_key (str): OCR.Space API key.
        language (str): Language code (default: 'eng').
    
    Returns:
        dict: {'text': extracted_text, 'error': error_message}
              - 'text': Raw text if successful.
...
def gemini_extract_details(raw_text, gemini_api_key=GEMINI_API_KEY):
    ...
    """
    try:
        if not genai or gemini_api_key == 'your-gemini-api-key-here':
            return {'error': 'Gemini API is not available or improperly configured.'}
...
def extract_complete_data(bill_image_path, warranty_image_path=None, ocr_api_key=OCR_SPACE_API_KEY, gemini_api_key=GEMINI_API_KEY):
    """
    Extracts complete raw text from bill and optional warranty images, then uses Gemini API to extract structured data.
    
    Args:
        bill_image_path (str): Path to the bill image file.
        warranty_image_path (str, optional): Path to the warranty image file.
        ocr_api_key (str): OCR.Space API key.
        gemini_api_key (str): Gemini API key.
    
    Returns:
        dict: {
            'structured_data': dict (extracted structured data),
            'error': str
        }
    """
    complete_data = ""
